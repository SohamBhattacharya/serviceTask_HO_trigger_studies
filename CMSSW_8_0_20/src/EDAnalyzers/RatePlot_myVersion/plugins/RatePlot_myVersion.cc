...
class Output
{
    ...
    TH2F *h_PU_vs_addTrack1_eta;
    TH2F *h_PU_vs_addTrack2_eta;
    TH2F *h_PU_vs_addTrackAll_eta;
    ...
    Output(edm::Service <TFileService> fs, const char *dirName)
    {
        ...
        sprintf(title, "PU_vs_addTrack1_eta");
        h_PU_vs_addTrack1_eta = dir.make <TH2F> (name, title, 34, -1.5, 1.5, 3000, 30, 60);
        
        sprintf(name, "PU_vs_addTrack2_eta");
        sprintf(title, "PU_vs_addTrack2_eta");
        h_PU_vs_addTrack2_eta = dir.make <TH2F> (name, title, 34, -1.5, 1.5, 3000, 30, 60);
        
        sprintf(name, "PU_vs_addTrackAll_eta");
        sprintf(title, "PU_vs_addTrackAll_eta");
        h_PU_vs_addTrackAll_eta = dir.make <TH2F> (name, title, 34, -1.5, 1.5, 3000, 30, 60);
...
class RatePlot_myVersion : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
    ...
    virtual double HOiEtaToEta(int iEta);
    ...
    TH2F *h_PU_vs_bmtfTrack1_eta;
    TH2F *h_PU_vs_bmtfTrack2_eta;
    TH2F *h_PU_vs_bmtfTrackAll_eta;
...
RatePlot_myVersion::RatePlot_myVersion(const edm::ParameterSet& iConfig)
{
    ...
    /*TFile *tfile = TFile::Open( \
        "root://eoscms.cern.ch//eos/cms/store/group/dpg_dt/comm_dt/dtRootple2016/Run2016HZMuPromptReco-v2.root", \
        "READ");
    TTree *dttree = (TTree*) tfile->Get("DTTree");
    printf("xxxxx %d xxxxx \n", (int) dttree->GetEntries());*/
    parseCSV();
    ...
    h_PU_vs_bmtfTrackAll_eta = fs->make <TH2F> ("PU_vs_bmtfTrackAll_eta", "PU_vs_bmtfTrackAll_eta", 34, -1.5, 1.5, 3000, 30, 60);
    
    h_mipHOTP = fs->make <TH1F> ("mipHOTP", "mip HOTP", 10, 0, 10);
...
void RatePlot_myVersion::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    iEvent.getByToken(tok_HOTriggerPrimitiveDigi, hotpHandle);
    ...
    double addTrack1_eta = -99;
    double addTrack2_eta = -99;
    
    double bmtfTrack1_pT = 0;
    double bmtfTrack2_pT = 0;
    
    double bmtfTrack1_eta = 0;
    double bmtfTrack2_eta = 0;
    ...
    for(unsigned int iDigiPh = 0; iDigiPh < digis_L1MuDTChambPh->size(); iDigiPh++)
    {
        ...
        globalPhi += M_PI / 6 * (digi_L1MuDTChambPh.scNum() - 1);
            ...
            hoTP != hotpHandle->end(); ++hoTP)
        {
            ...
            for(l1t::RegionalMuonCandBxCollection::const_iterator muonCand = bmtfHandle->begin(); muonCand != bmtfHandle->end(); ++muonCand)
            {
                ...
                // Match HO-TP to BMTF track within a 3x3 window
            ...
            if(!isPresent)
            {
                ...
                // Get eta from matched HO-TP
                double muDT_eta = HOiEtaToEta(hoTP->ieta());
                
                
                printf("addTrackAll: %f %f \n", muDT_pT, muDT_eta);
                currentOutput->h_PU_vs_addTrackAll_pT->Fill(muDT_pT, pileUp);
                currentOutput->h_PU_vs_addTrackAll_eta->Fill(muDT_eta, pileUp);
                ...
                if(muDT_pT > addTrack1_pT)
                {
                    ...
                    addTrack2_eta = addTrack1_eta;
                    addTrack1_eta = muDT_eta;
    ...
    for(l1t::RegionalMuonCandBxCollection::const_iterator muonCand = bmtfHandle->begin(); \
        muonCand != bmtfHandle->end(); ++muonCand)
    {
        double bmtf_pT = muonCand->hwPt() * 0.5;
        double bmtf_eta = muonCand->hwEta() * 0.010875;
        
        printf("bmtfTrackAll: %f %f \n", bmtf_pT, bmtf_eta);
        h_PU_vs_bmtfTrackAll_pT->Fill(bmtf_pT, pileUp);
        h_PU_vs_bmtfTrackAll_eta->Fill(bmtf_eta, pileUp);
        ...
        if(bmtf_pT > bmtfTrack1_pT)
        {
            ...
            bmtfTrack2_eta = bmtfTrack1_eta;
            bmtfTrack1_eta = bmtf_eta;
        }
        
        else if(bmtf_pT > bmtfTrack2_pT)
        {
            bmtfTrack2_pT = bmtf_pT;
            bmtfTrack2_eta = bmtf_eta;
        }
        ...
        printf("gmtStage2Digis %f %f %f \n", ptTrack, etaTrack, phiTrack);*/
    }
            ...
            hoTP != hotpHandle->end(); ++hoTP)
    {
        ...
        h_mipHOTP->Fill(isMip);
        
        //printf("HOTP: %d %f %d \n", hoTP->ieta(), HOiEtaToEta(hoTP->ieta()), hoTP->iphi());
    }
    ...
    if(addTrack1_pT > 0)
    {
        ...
        currentOutput->h_PU_vs_addTrack1_eta->Fill(addTrack1_eta, pileUp);
    }
    ...
    if(addTrack2_pT > 0)
    {
        ...
        currentOutput->h_PU_vs_addTrack2_eta->Fill(addTrack2_eta, pileUp);
    }
    ...
    if(bmtfTrack1_pT > 0)
    {
        ...
        h_PU_vs_bmtfTrack1_eta->Fill(bmtfTrack1_eta, pileUp);
    }
    ...
    if(bmtfTrack2_pT > 0)
    {
        ...
        h_PU_vs_bmtfTrack2_eta->Fill(bmtfTrack2_eta, pileUp);
    }
}
...
double RatePlot_myVersion::HOiEtaToEta(int iEta)
{
