...
int TTreeGenerator::getIphi(std::vector<L1MuDTChambPhDigi>::const_iterator digi_L1MuDTChambPh)
{
    ...
    phiGlobal += 2.0*M_PI/12.0 * digi_L1MuDTChambPh->scNum();
...
void TTreeGenerator::fill_twinmuxth_variables(edm::Handle<L1MuDTChambThContainer> localTriggerTwinMux_Th)
{
    ...
    for(std::vector<L1MuDTChambThDigi>::const_iterator ith = thTrigs->begin(); ith != thTrigs->end(); ++ith)
    {
        ...
        for(int i = 0; i < 7; i++)
        {
            ...
            ltTwinMux_thPosition.push_back(i);
            ltTwinMux_thQuality.push_back(ith->quality(i));
            ltTwinMux_thIeta.push_back(getIeta(ith, i));
        }
    }
    return;
}
...
int TTreeGenerator::getIeta(std::vector<L1MuDTChambThDigi>::const_iterator digi_L1MuDTChambTh)
{
    ...
    for(int i = 0; i < 7; i++)
    {
        ...
        if(digi_L1MuDTChambTh->position(i) == 1)
        {
            if(digi_L1MuDTChambTh->quality(i) == 1)
                nHQ++;
            else
                nLQ++;
            
            printf("### bit %d, qua %d \n", i, digi_L1MuDTChambTh->quality(i));
        }
    }
    
    printf("*** nLQ %d, nHQ %d \n", nLQ, nHQ);
    
    return getIeta(digi_L1MuDTChambTh, pos);
}


int TTreeGenerator::getIeta( \
    ...
    int pos)
{
    ...
    if(abs(digi_L1MuDTChambTh->whNum()) == 2)
    {
        return (int) (round(sign1*15 + sign2*5.0/7.0*pos));
    }
    
    if(abs(digi_L1MuDTChambTh->whNum()) == 1)
    {
        return (int) (round(sign1*10 + sign2*6.0/7.0*pos));
    }
    
    if(abs(digi_L1MuDTChambTh->whNum()) == 0)
    {
        return (int) (round(sign1*4 + sign2*8.0/7.0*pos));
    }
